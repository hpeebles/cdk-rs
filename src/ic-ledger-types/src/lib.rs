...
impl Transaction {
    pub fn hash(&self) -> [u8; 32] {
        // To match the transaction hashes generated by the ledger, the transaction must be
        // serialized with the AccountIdentifiers serialized as strings.
        // So this `TransactionForHash` type matches the `Transaction` type exactly, except with
        // each `AccountIdentifier` replaced with a `String`.
        #[derive(Serialize)]
        struct TransactionForHash {
            operation: OperationForHash,
            memo: Memo,
            created_at_time: Timestamp,
        }

        #[derive(Serialize)]
        enum OperationForHash {
            Burn {
                from: String,
                amount: Tokens,
            },
        ...
        let transaction_for_hash = TransactionForHash {
            operation: match self.operation {
                Operation::Burn { from, amount } => OperationForHash::Burn { from: from.to_string(), amount },
                Operation::Mint { to, amount } => OperationForHash::Mint { to: to.to_string(), amount },
                Operation::Transfer {
                    from,
                    to,
                    amount,
                    fee
                } => OperationForHash::Transfer {
                    from: from.to_string(),
                    to: to.to_string(),
                    amount,
                    fee
        ...
        hasher.update(&serde_cbor::ser::to_vec_packed(&transaction_for_hash).unwrap());
